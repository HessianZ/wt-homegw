    ...
    while (xQueueReceive(s_example_espnow_queue, &evt, portMAX_DELAY) == pdTRUE) {
        switch (evt.id) {
            ...
            case EXAMPLE_ESPNOW_RECV_CB:
            {
                ...
                ESP_LOGI(TAG, "Report{version: %d, battery: %d, temperature: %.2f, humidity: %.2f, light: %d, earthHumidity: %d}",
                        report.version, report.battery, report.temperature, report.humidity, report.light, report.earthHumidity);
